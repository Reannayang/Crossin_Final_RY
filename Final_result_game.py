...
with open("game_record.txt","w+",encoding='utf-8') as f:
    ...
    if username in users:
        ...
        history_no = [0,0,0]
    f.close()
print(username, 'you have been played %d times, at least caught in %d times and at average caught in %d times'%(history_no[0],history_no[1],history_no[2]) )
...
def one_round():
    ...
    times = 0
    while bingo == False:
        trial = int(input('plz guess a number between 1 and 100'))
        if trial > answer:
            print('too big')
        elif trial< answer:
            print('too small')
...
try:
    ...
    with open('game_record.txt','r+',encoding='utf-8') as f:
        f.writelines(j for j in history_info)
        f.close()
except:
    print('this is not valid value.plz try again')
